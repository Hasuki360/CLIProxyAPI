...
func geminiToAntigravity(modelName string, payload []byte, projectID string, derivedSessionIDs ...string) []byte {
	...
	template, _ = sjson.DeleteBytes(template, "request.messages")
	template, _ = sjson.DeleteBytes(template, "messages")
	template, _ = sjson.DeleteBytes(template, "request.temperature")
	template, _ = sjson.DeleteBytes(template, "temperature")
	template, _ = sjson.DeleteBytes(template, "request.stream")
