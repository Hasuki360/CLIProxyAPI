...
func TestConvertGeminiRequestToAntigravity_NormalizesOpenAIPayloadWithMessagesAndTemperature(t *testing.T) {
	inputJSON := []byte(`{
		"model": "fanyi",
		"messages": [
	...
	out := ConvertGeminiRequestToAntigravity("gemini-3.1-flash-lite", inputJSON, false)

	// request.messages and request.temperature should be gone
	if gjson.GetBytes(out, "request.messages").Exists() {
		t.Fatalf("request.messages should have been removed: %s", out)
	}
	if gjson.GetBytes(out, "request.temperature").Exists() {
		t.Fatalf("request.temperature should have been removed: %s", out)
	}
	if gjson.GetBytes(out, "request.stream").Exists() {
		t.Fatalf("request.stream should have been removed: %s", out)
	}

	// generationConfig should contain temperature and topP
	if gjson.GetBytes(out, "request.generationConfig.temperature").Float() != 0.7 {
		t.Fatalf("expected generationConfig.temperature to be 0.7: %s", out)
	}
	if gjson.GetBytes(out, "request.generationConfig.topP").Float() != 0.95 {
		t.Fatalf("expected generationConfig.topP to be 0.95: %s", out)
	}

	// systemInstruction should exist
	sysText := gjson.GetBytes(out, "request.systemInstruction.parts.0.text").String()
	if sysText != "You are a professional translator." {
		t.Fatalf("expected systemInstruction text, got %q", sysText)
	}

	// contents should contain user turn
	userText := gjson.GetBytes(out, "request.contents.0.parts.0.text").String()
	if userText != "Hello world" {
		t.Fatalf("expected contents text 'Hello world', got %q", userText)
	}
}
